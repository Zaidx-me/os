...
export default function MusicApp({ windowId }) {
  ...
  const profileName = "Muhammad Zaid";
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        ...
        {isMobile ? (
          <div
            data-testid="music-mobile-player"
            className="music-mobile-player absolute inset-x-3 bottom-3 z-40 flex items-center gap-3 rounded-2xl border px-3 py-2.5 shadow-xl backdrop-blur-xl"
            style={{
              background: isDarkMode ? "rgba(28,28,30,0.92)" : "rgba(255,255,255,0.92)",
              borderColor: isDarkMode ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.08)",
              paddingBottom: "max(0.625rem, env(safe-area-inset-bottom))",
            }}
          >
            <img src={currentSong.img} alt="" className="h-11 w-11 shrink-0 rounded-lg object-cover shadow-sm" />
            <div className="min-w-0 flex-1">
              <p className="truncate text-[13px] font-semibold text-gray-900 dark:text-white">{currentSong.title}</p>
              <p className="truncate text-[11px] text-gray-500 dark:text-gray-400">{currentSong.artist}</p>
            </div>
            <div className="flex shrink-0 items-center gap-1">
              <button
                type="button"
                onClick={() => playSong(currentSongIndex > 0 ? currentSongIndex - 1 : songs.length - 1)}
                className="flex h-9 w-9 items-center justify-center rounded-full text-gray-700 dark:text-gray-200"
                aria-label="Previous"
              >
              ...
              <button
                type="button"
                onClick={togglePlay}
                className="flex h-10 w-10 items-center justify-center rounded-full bg-rose-500 text-white shadow-md"
                aria-label={storeIsPlaying ? "Pause" : "Play"}
              >
                {storeIsPlaying ? <FiPause size={14} className="fill-current" /> : <FiPlay size={14} className="fill-current ml-0.5" />}
              </button>
              <button
                type="button"
        ...
        <div 
          className={`music-desktop-player absolute bottom-6 left-1/2 -translate-x-1/2 w-[620px] h-[52px] rounded-full border shadow-2xl flex items-center justify-between px-5 z-40 transition-all ${
