...
import { buildPortfolioFinderFiles } from "../zaidos/lib/portfolio-files.js";
import { useOpenApp } from "../zaidos/lib/openApp.js";
import { useIsMobile } from "../hooks/useIsMobile.js";
import { FINDER_ICONS, getAppIcon } from "../zaidos/lib/appIcons.js";
import AppIconImg from "../components/AppIconImg.jsx";
import {
  Clock,
  Users,
  Download,
...
// Folder / file icons (local assets)

// Portfolio files from ZaidOS content layer (real projects + articles)
const initialICloudFiles = buildPortfolioFinderFiles();
...
export default function Finder({ initialPath = "/icloud", windowId, maximized, isDragging, isResizing }) {
  ...
  const saveTextFile = (fileId, newTitle, newText) => {
    ...
    setZaidFiles((prev) => (prev.some((f) => f.id === fileId) ? updateFilesList(prev) : prev));
  ...
  const [zaidFiles, setZaidFiles] = useState(() => {
    const saved = localStorage.getItem("os_zaid_files");
    const parsed = saved ? JSON.parse(saved) : [];
    return parsed.filter(f => f.id !== "zaid_welcome");
  });
  ...
  useEffect(() => {
    localStorage.setItem("os_zaid_files", JSON.stringify(zaidFiles));
  }, [zaidFiles]);
  ...
  useEffect(() => {
    ...
    const handleFilesSync = () => {
      ...
      const zaid = JSON.parse(localStorage.getItem("os_zaid_files") || "[]");
      ...
      setZaidFiles(prev => JSON.stringify(prev) === JSON.stringify(zaid) ? prev : zaid);
    };
  ...
  const deleteFile = (fileId) => {
    ...
    targets.forEach(fileToTrash => {
      ...
      } else if (currentPath === "/zaid") {
        setZaidFiles(prev => prev.filter(f => f.id !== fileToTrash.id));
      }
    });
  ...
  const addFileToCurrentDirectory = (newFile) => {
    if (currentPath === "/icloud") {
      ...
      setZaidFiles(prev => [...prev, newFile]);
    }
  };
  ...
  const handlePaste = () => {
    ...
    if (currentIsCutMode) {
      clipboardList.forEach((clipboardItem) => {
        ...
        setZaidFiles(prev => removeFromFileList(prev));
        ...
        const zaid = removeFromFileList(JSON.parse(localStorage.getItem("os_zaid_files") || "[]"));
        ...
        localStorage.setItem("os_zaid_files", JSON.stringify(zaid));
  ...
  const handleRename = (file) => {
    ...
    showCustomPrompt("rename", "Rename Item", file.name, (newName) => {
      ...
      if (currentPath === "/icloud") {
        ...
        setZaidFiles(prev => prev.map(f => f.id === file.id ? updatedFile : f));
      }
  ...
  const handleOpenFile = (file) => {
    if (file.type === "folder") {
      ...
      if (isMobile) {
        window.dispatchEvent(new CustomEvent("zaidos:open-editor", { detail: { file } }));
      } else {
        openApp("TextEdit", <TextEdit file={file} />);
      }
    } else if (file.type === "pdf") {
      if (isMobile) {
        window.dispatchEvent(new CustomEvent("zaidos:open-pdf", { detail: { file } }));
      } else {
        openApp("PDFViewer", <PDFViewer file={file} />);
      }
    } else if (file.type === "app") {
      if (isMobile && file.appId) {
        window.dispatchEvent(new CustomEvent("zaidos:open-app", { detail: { appId: file.appId } }));
        return;
      }
      if (file.url) {
