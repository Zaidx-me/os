...
export default function AppWindow({ window: win }) {
  ...
  const NATIVE_CHROME_APPS = new Set([
    "Photos", "Music", "Safari", "Notes", "Finder", "TextEdit", "PDFViewer", "Trash",
    "Launchpad", "Messages", "Mail", "Maps", "FaceTime", "Phone", "Calendar", "Contacts", "Reminders",
  ]);
  const usesGenericTitleBar = !NATIVE_CHROME_APPS.has(win.appId);
  const isGlassShell = win.appId === "Launchpad" || usesGenericTitleBar;

  const glassShellClasses = isDarkMode
  ...
  // Unmount app content when minimized (except Music — keeps audio alive).
  const keepMountedWhenMinimized = win.appId === "Music";
  const showContent = !win.minimized || keepMountedWhenMinimized;
        ...
        <AnimatePresence onExitComplete={handleExitComplete}>
          {isVisible && (
            <motion.div
              ...
              } ${isGlassShell ? "app-window-glass" : ""} ${isDragging ? "cursor-grabbing" : ""} ${isResizing ? "select-none" : ""}`}
              style={{
                width: '100%',
                height: '100%',
            ...
            >
              ...
              {usesGenericTitleBar && (
                <div 
                  className={`window-drag-handle relative z-10 flex h-11 shrink-0 items-center border-b cursor-grab active:cursor-grabbing select-none backdrop-blur-xl ${
                ...
                >
                  ...
                  {/* Center Title */}
                  <div className={`absolute left-1/2 -translate-x-1/2 font-medium text-sm tracking-wide pointer-events-none ${
              ...
              {/* App Content */}
              <div className="app-window-content flex min-h-0 flex-1 flex-col overflow-hidden">
                {showContent && React.cloneElement(win.component, { windowId: win.id, maximized: win.maximized, isDragging, isResizing })}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
