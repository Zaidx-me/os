...
export default function Desktop({ setStage, isLocked = false }) {
  ...
  useEffect(() => {
    ...
    const handleWallpaperChange = (e) => {
      const newWallpaper = resolveWallpaper(e?.detail || localStorage.getItem("desktop_wallpaper"));
  ...
  const handleContextMenu = (e) => {
    e.preventDefault();
    if (e.target.closest("[data-desktop-item]")) return;
    if (e.target.closest(".mac-dock, .mac-dock-host, .mac-dock-inner, [data-app-window], .top-bar")) return;
  ...
  return (
    <LazyWallpaper
      ...
      className="relative w-screen h-screen max-w-screen max-h-screen overflow-hidden desktop-area"
