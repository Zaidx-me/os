...
import { resolveLockWallpaper, DEFAULT_LOCK_WALLPAPER } from "../zaidos/lib/assets.js";
...
export default function LockScreen({ goNext, isLocked }) {
  ...
  const [lockscreenWallpaper, setLockscreenWallpaper] = useState(() => {
    return resolveLockWallpaper(localStorage.getItem("lockscreen_wallpaper"));
  });
  ...
  // User profile state
  const [username, setUsername] = useState(() => localStorage.getItem("lock_username") || "Muhammad Zaid");
  ...
  // Preload optimized lock wallpaper only (small WebP)
  useEffect(() => {
    const img = new Image();
    img.decoding = "async";
    img.src = resolveLockWallpaper(localStorage.getItem("lockscreen_wallpaper"));
  }, []);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage: `url('${resolveLockWallpaper(lockscreenWallpaper)}')`,
          zIndex: 0,
        }}
      />
      ...
      {depthEnabled && (
        <div
          ...
          style={{
            backgroundImage: depthForeground ? `url('${depthForeground}')` : `url('${resolveLockWallpaper(lockscreenWallpaper)}')`,
            zIndex: 10,
            ...(depthForeground ? {} : {
